    ...
    with dbapi2.connect(app.config['dsn']) as connection:
        ...
        query3 =""" SELECT matches.t_id,team1name,t_id_2,TEAM.t_name as team2name,t_1_score,t_2_score,br_name,matches.br_id,br_stage,br_index,COALESCE(m_index,0)
                    FROM (
                            ...
                            FROM (
                                    SELECT t_id,t_id_2,br_name,t_1_score,t_2_score,br_stage,br_index,m_index,BRACKET.br_id
                                    FROM BRACKET
                                    LEFT JOIN MATCH
...
@app.route('/json/bracket/<name>')
def jsonbracket(name):
    with dbapi2.connect(app.config['dsn']) as connection:
        cursor = connection.cursor()
        query = """ SELECT br_id,br_name,tr_name FROM BRACKET LEFT JOIN TOURNAMENT ON BRACKET.tr_id = TOURNAMENT.tr_id
        WHERE LOWER(br_name) LIKE LOWER(%s)"""
        cursor.execute(query,['%'+name+'%'])
        result = cursor.fetchall()
        resultList = []
        for i in result:
            resultList.append({"id":i[0],"name":i[1] +", " + i[2]})
        connection.commit()
    return jsonify(resultList)
